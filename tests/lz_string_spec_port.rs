    ...
    D: Fn(ByteString) -> Option<ByteString>,
{
    ...
    */
    ...
    if !bytearray {
        ...
        todo!("Add bytearray testing support");
        // expect(compressed instanceof Uint8Array).toBe(true);
...
fn lz_string_uri_encoded() {
    compression_tests(
        |s| ByteString::from(lz_str::compress_uri(s)),
        |s| lz_str::decompress_uri(&s.0).map(ByteString::from),
        false,
    );
}
...
fn lz_string_raw() {
    compression_tests(
        |s| ByteString::from(lz_str::compress_str(s)),
        |s| lz_str::decompress_str(&s.0).map(ByteString::from),
        false,
    );
}
...
fn specific_url_encoded() {
    ...
    let compressed = lz_str::compress_uri(&test_string);
    assert!(!compressed.contains(&u32::from(b'=')));
    assert!(!compressed.contains(&u32::from(b'/')));
    let decompressed = lz_str::decompress_uri(&compressed).expect("Valid Decompress");
    assert_eq!(decompressed, test_string);
    ...
    let decomp2 = lz_str::decompress_uri(&compressed.chars().map(u32::from).collect::<Vec<u32>>())
        .expect("Valid Decompress");
    assert_eq!(decompressed, decomp2);
}

fn test_enc_bin_compat<C>(comp: C, expected_dec: &str, expected_comp: &str)
where
    C: Fn(&str) -> ByteString,
{
    ...
    for (i, (e, a)) in expected_comp
        .chars()
        .zip(compressed.0.iter().copied())
        .enumerate()
    {
        if u32::from(e) != a {
            diffs.push((i, e, a));
    ...
    if !diffs.is_empty() {
        for (i, e, a) in diffs {
            eprintln!("Location: {}, expected: {}, actual: {}", i, e, a);
        }

        panic!("Actual does not match expected in `test_enc_bin_compat`");
    }

    assert_eq!(compressed.len(), expected_comp.chars().count());
}

fn test_dec_bin_compat<D>(dec: D, expected_dec: &str, expected_comp: &str)
where
    D: Fn(ByteString) -> Option<String>,
{
    let decompressed = dec(expected_comp.into()).expect("Valid Decompress");
...
fn binary_encoding_compatibility_tests_optional() {
    ...
    */
    ...
    test_enc_bin_compat(
        |s| lz_str::compress_to_utf16(s).into(),
        utf16_decompressed,
        utf16_compressed,
    );
}
...
fn binary_decoding_compatibility_tests_mandatory() {
    ...
    */
    ...
    test_dec_bin_compat(
        |s| lz_str::decompress_from_utf16(&s.to_utf8_string().expect("Valid UTF16")),
