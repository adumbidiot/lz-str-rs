...
var LZString = {
  ...
  compress: function (uncompressed) {
	...
	console.log(context.data.string);
    return context.data.string;
  },
  
  readBit : function(data) {
    var res = data.val & data.position;
	console.log(res, data.position, data.val);
    data.position >>= 1;
    if (data.position == 0) {
      data.position = 32; //32768
  ...
  decompress: function (compressed) {
	...
	//console.log(c, dictionary[c].charCodeAt(0));
      if (dictionary[c]) {
        entry = dictionary[c];
      } else {
		console.log("C", c, dictSize);
        if (c === dictSize) {
